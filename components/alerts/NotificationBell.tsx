...
export default function NotificationBell({ orgId }: { orgId: string }) {
  ...
  const fetchNotifications = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/alerts/notifications?org_id=${orgId}&status=active&limit=10`);
      const data = await res.json();
      ...
      {open && (
        <div className="absolute left-full ml-2 bottom-0 w-96 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">{count} active</span>
          </div>
          ...
          <div className="px-4 py-2 bg-gray-50 border-t border-gray-200">
            <a href="/alerts" className="text-xs text-indigo-600 hover:text-indigo-800">
              View all alerts →
            </a>
          </div>
        </div>
