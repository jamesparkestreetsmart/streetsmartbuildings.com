...
function displayProjectCode(code: string): string {
  if (!code) return code;
...
export default function ProjectDetailPage({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      if (projRes.error) {
        setError("Failed to load project: " + projRes.error.message);
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div>
        ...
        {project.notes && <p className="text-sm text-gray-500 mt-1">{project.notes}</p>}
      </div>
